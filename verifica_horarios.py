...
def esta_disponivel(intervalos, hora_minutos):
    for intervalo in intervalos.split(","):
        if intervalo.strip().lower() == "livre":
            return True
        try:
...
" + "\n".join(disponiveis) if disponiveis else "Ninguém disponível")
    st.error("Ocupados:
" + "\n".join(ocupados) if ocupados else "Ninguém ocupado")